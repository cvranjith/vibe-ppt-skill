...
// data: { line1, line2, subtitle?, context?: { title, text, accentColor? }, branding? }
function addTitleSlide(pres, data) {
  const slide = pres.addSlide();
  const branding = data.branding || {};
  slide.background = { color: BRAND.slideBg };

  addBrandingBg(slide, { ...branding, type: "title" });

  // Skip ovals when abstract bg is used (they conflict)
  if (!branding.abstractBg) {
